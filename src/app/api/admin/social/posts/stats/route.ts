...
export async function GET(req: NextRequest) {
  try {
    const supabase = await createServerClient() // ⭐ AJOUT de await
    ...
    const revenueByType: Record<string, number> = {
      story: posts
        ...
        .reduce((sum: number, p: PostPerformance) => sum + p.web_revenue, 0),
      feed: posts
    ...
    const bestDayEntry = Object.entries(postsByDay)
      ...
      .sort((a, b) => {
        const [_aDay, aStats] = a as [string, DayStats]
        const [_bDay, bStats] = b as [string, DayStats]
    ...
    const avgEngagement =
      posts.reduce(
        (sum: number, p: PostPerformance) => sum + p.engagement_rate,
        0
      ) / posts.length

    const avgConversion =
      posts.reduce(
        (sum: number, p: PostPerformance) => sum + p.web_conversion_rate,
        0
      ) / posts.length
